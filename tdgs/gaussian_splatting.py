...
class VLSRender:
    ...
    def rendering(self, means3D, scales, quats, colors, opacities, device='cuda'):
        ...
        raster_settings = GaussianRasterizationSettings(
            ...
            scale_modifier=1.0,
            viewmatrix=self.camera.world_view_transform.to(device),
            projmatrix=self.camera.full_proj_transform.to(device),
            sh_degree=3,  # not use
            campos=self.camera.camera_center.to(device),
            prefiltered=False,
            debug=False
        )
...
def render(viewpoint_camera, pc, scaling_modifier=1.0):
    ...
    raster_settings = GaussianRasterizationSettings(
        ...
        # viewmatrix=viewpoint_camera.full_proj_transform,
